...
def solveq2(data):
    # permutations_tuple = tuple(permutations(set(range(10)) - set(data[0]) - set(data[1]) - set(data[2])))
    ...
    return int(max(list(map(lambda x: sum(x) / len(x),
                        [[score_mapping[sum(case[0])] for case in cases_tuple],
                         [score_mapping[sum(case[1])] for case in cases_tuple],
                         [score_mapping[sum(case[2])] for case in cases_tuple],
                         [score_mapping[case[0][0] + case[1][0] + case[2][0]] for case in cases_tuple],
                         [score_mapping[case[0][1] + case[1][1] + case[2][1]] for case in cases_tuple],
                         [score_mapping[case[0][2] + case[1][2] + case[2][2]] for case in cases_tuple],
                         [score_mapping[case[0][0] + case[1][1] + case[2][1]] for case in cases_tuple],
                         [score_mapping[case[0][2] + case[1][1] + case[2][0]] for case in cases_tuple],
                         ]))))


def fill_in_zeros(data, permutation):
...
def main():
    ...
    print(solveq2([[0, 0, 0], [0, 0, 0], [0, 0, 0]]))
